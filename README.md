## Bridging Synthetic and Real-World Graph Scenarios via Integer Programming:  A Spatial Optimization Perspective on the Signed Double Roman Domination Problem

This repository contains benchmark instances and raw experimental results for three integer linear programming (ILP) models developed to solve the **Signed Double Roman Domination (SDRD)** problem.

This repository accompanies the research paper:

**“Bridging Synthetic and Real-World Graph Scenarios via Integer Programming: A Spatial Optimization Perspective on the Signed Double Roman Domination Problem”**  
by *Dragan Matić and Marko Djukanović*.

For additional information, please contact the corresponding author:  
**dragan.matic@pmf.unibl.org**

---

## Repository Structure

- **instances/** — Contains four groups of graph instances used in the study:
- random graphs  
- trees  
- wireless-network graphs  
- urban-network graphs  

- **results-all/** — Contains the raw computational results of the three ILP models reported in the paper.
